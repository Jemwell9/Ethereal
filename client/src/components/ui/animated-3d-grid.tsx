...
export function Animated3DGrid({ variant = "default" }: { variant?: "default" | "printer" | "scanner" }) {
  ...
  // Track if we're below the hero section
  useEffect(() => {
    const checkScroll = () => {
      const heroHeight = window.innerHeight; // Assuming hero is full viewport height
      setIsBelowHero(window.scrollY > heroHeight * 0.8); // Start showing slightly before leaving hero
    };
  ...
  const y = useTransform(scrollY, 
    [0, 1000], 
    [0, -200]  // Parallax effect
  );

  if (!isBelowHero) return null;

  return (
    <motion.div 
      className="fixed inset-0 overflow-hidden pointer-events-none"
      style={{ opacity: isBelowHero ? 1 : 0 }}
      initial={{ opacity: 0 }}
